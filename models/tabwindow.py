...
class TabularSeqEncoder(nn.Module):
    ...
    def forward(self, x: Tensor) -> Tensor:
        """
        :param x:       [nwindow, nstep, nfield], LongTensor
        :return:        [nwindow, nquery, nemb], FloatTensor
        """
        nwindow = x.size(0)
        # arm
        x = self.feature_embedding(x)                               # nwindow*nstep*nfield*nemb
        x = rearrange(x, 'b t f e -> (b t) f e')                    # (nwindow*nstep)*nfield*nemb
...
class TabWindow(nn.Module):
    ...
    def forward(self, features) -> Tensor:
        """
        :param features:    [sequential, quantitative, semantic, tabular], each of [nwindow, *], LongTensor/FloatTensor
