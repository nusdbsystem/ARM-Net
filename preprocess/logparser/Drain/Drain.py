...
class LogParser:
    ...
    def parse(self, logName):
        ...
        for idx, line in self.df_log.iterrows():
            ...
            if 'HDFS' in self.logName:
                blk, content = self.preprocess(line['Content'])
                lst_blk.append(blk)
            else:
                content = self.preprocess(line['Content'])
            logmessageL = content.strip().split()
    ...
    def preprocess(self, line):
        if 'HDFS' in self.logName:
            pattern = re.compile(self.rex[0])
            lst_blk = pattern.findall(line)
            blk = lst_blk[0][0:-1] if lst_blk[0][-1] == '.' else lst_blk[0]
